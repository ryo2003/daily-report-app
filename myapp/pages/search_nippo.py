...
import streamlit as st
import asyncio
def select_nippo(nippos,sel_username):
    result_str = '<html><table style="border: none; width: 100%;">'
    for nippo in nippos:
        if(get_username(nippo.user_id) == sel_username):
...
def show_nippo(nippos, sel_username=None):
    result_str = '<html><table style="border: none; width: 100%;">'
    for nippo in nippos:
        username = get_username(nippo.user_id)
        users.append(username)
...
async def main():
    ...
    selected_name = st.sidebar.selectbox("報告者を選択してください", options=["すべて"] + data.get("報告者"))
    selected_company = st.sidebar.selectbox("企業名を選択してください", options=["すべて"] + data.get("企業名"))
    selected_purpose = st.sidebar.selectbox("訪問目的を選択してください", options=["すべて"] + data.get("訪問目的"))
